...
<body>
  ...
  <form action="/manual">
    <label>Hours</label>
    <input type="number" min="0" max="'numleds'" name="last">
    <label>Minutes</label>
    <input type="color" min="" max="59" name="color">
    <label>Duration</label>
    <input type="number" min="0" max="59" name="duration">
    <input type="submit" class="submit" value="Display">
  </form>
</body>
<script>
  ...
  function setTimeBar() {
    ...
    var format = 11;
    if (document.getElementById("format").checked) format = 23;

    var hoursTime = time.getHours();
    if (hoursTime >= 12 && format == 11) hoursTime -= 12;

    minutes.style.width = (100 * (time.getMinutes() / 60)) + "%";
    hours.style.marginLeft = (100 * (hoursTime / format)) + "%";
